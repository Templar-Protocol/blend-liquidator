...
async fn seed_pools_needing_it(
    ...
    for pool in pools {
        ...
        let outcome = tracker.seed(&pool.address, sources, tick, batch).await?;
        tracing::info!(
            pool = pool.address,
...
async fn handle_message(
    ...
    match message {
        PollerMessage::Event {
            ...
            let accounts = tracker.apply(&pool, ledger, &event).await?;
            pending.entry(pool).or_default().extend(accounts);
        }
        PollerMessage::Tick { pool, tick } => {
            let accounts: Vec<String> = pending.remove(&pool).into_iter().flatten().collect();
            tracker.refresh(&pool, &accounts, tick).await?;
            tracker
                .refresh_stale(
                    &pool,
                    tick,
                    cadence.user_refresh_ledgers,
                    cadence.refresh_batch,
                )
                .await?;
        ...
        PollerMessage::Gap { pool, from, oldest } => {
            ...
            tracker
                .seed(&pool, seed_sources, tick, cadence.refresh_batch)
                .await?;
        }
    }
    Ok(())
...
impl Service {
    ...
    pub async fn run(config: ServiceConfig) -> Result<(), LiquidatorError> {
        ...
        tasks.spawn(async move {
            tracker_loop(&rpc, &store, &seed_sources, cadence, message_rx)
                .await
                .map_err(LiquidatorError::from)
        });
