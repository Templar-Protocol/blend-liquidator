...
- **Image.** Published to `ghcr.io/templar-protocol/blend-liquidator`
  ...
  neither. The repository is private, so the published package is
  private too: pulling it needs a token with `read:packages`.
- **Both build stages pin their base image by digest**, not only by tag,
  so the same source revision always produces the same image regardless
  of what the tag later resolves to.
- **The image runs as a non-root user** (`liquidator`, uid 1000).
- **Configuration arrives only through the environment and the pools
  file or inline string** (`POOLS_FILE`/`POOLS_TOML`). The process
  writes no filesystem state of its own — nothing to persist or back up
  beyond the database.
...
- **`RUN_MODE=check-config` is a deploy smoke test.** In order, it:
  ...
  against chain; validates the filler account (existence and holding at
  least `XLM_FEE_RESERVE`); and, when Telegram is configured, calls
  `getMe` to prove the bot token works. It exits `0` on success and `2`
  on any failure, changing nothing on either path — no migration runs
...
- **Exactly five environment variables are ever secret**:
  ...
  from the command line, and none is ever rendered in a log line: the
  two signing keys render as their public address only, and the other
  three render as `Secret(<redacted>)`.
- **Exit codes**: `0` on graceful shutdown or a passing `check-config`;
  `2` on a configuration error, including a failed `check-config`; `1`
  on any other fatal error while running; `130` on a second
  `SIGINT`/`SIGTERM` — deliberately immediate, and the one exit that
  skips draining notifications still in flight.
- **Logs go to stdout**, one line per event; `LOG_FORMAT=json` renders
  each as one JSON object per line instead of text.
...
- **`STARTUP_DELAY_LEDGERS` set above the previous revision's shutdown
  drain, for any deployment that runs overlapping revisions.** It
  defaults to `0`: the image does not keep that overlap window empty on
  its own, and a rolling deploy that never sets it can have two
  revisions submitting at once until the older one finishes draining.
- **The image run with `DRY_RUN=true` — its own default — until
  `RUN_MODE=check-config` passes against the real configuration.** Only
  then is turning it to `false` for that deployment a decision the
  operator has evidence for.
