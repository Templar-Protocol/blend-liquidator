...
6. **The unwind floor is the pool's `min_health_factor` alone**, as spec §5 step 3 says; `HF_SAFETY_MULTIPLIER` is the *fill's* margin.
7. **The 1%-of-floor dust rule applies to every partial withdrawal of the primary, in step 2 as well as step 3**, and `WithdrawAll` of a non-primary asset is never dust. The 0.5% rule ends step 3's search: once the projected health factor is within 0.5% of the minimum, nothing further is withdrawn.
