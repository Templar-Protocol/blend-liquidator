...
8. **A dry-run fill is recorded once per auction**, keyed `(pool, account, start_ledger)` in memory, not on every tick until someone else fills it. A restart may record one more; nothing else does.
...
| §8 reservations held until the outcome is known; a drop guard settles every non-panicking path | 5, 6, 7 |
