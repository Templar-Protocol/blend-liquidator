...
- Round 4: a restore whose `send` fails after the RPC may have accepted the envelope (a transport, HTTP, JSON-RPC or echoed-hash failure) returns `ChainError::RestoreUnknown` with the restore's handle, while `BadSequence` and `Rejected` still propagate; the bounds test reads the clock once before `prepare` and tolerates a minute of delay.
